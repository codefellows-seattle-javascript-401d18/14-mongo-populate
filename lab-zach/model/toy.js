...
const Toy = mongoose.Schema({
  ...
  child: {type: mongoose.Schema.Types.ObjectId, require : true, ref: 'child'},
}, {timestamps: true});
...
Toy.pre('save', function(next) {
  ...
  Child.findById(this.child)
    ...
    .then(next)
    .catch( () => {next(new Error ('validation failed to create toy : child does not exist'));});
});

Toy.post('remove', function(doc, next){
  debug('#post-Toy-removal');
